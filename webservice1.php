...
switch ($case) {
    case 'CommonLogin':
      ...
      if ($count == 1) {
         ...
         $string = "response_message.Login success,response_code.1";
				...
				$string = "response_message.Invalid Credential,response_code.0";
				$a = explode(',', $string);
				foreach ($a as $result) {
					$b = explode('.', $result);
					$array[$b[0]] = $b[1];
    ...
    case 'Registration':
    	...
    	if($count6='0'){
        $sql2 = "INSERT INTO executive_details(team_leader_id,employee_id,dispo_code,executive_summary,executive_other,executive_location_lat,executive_location_long,executive_address,executive_time,executive_transtype) VALUES ('".$pval['status_id']."','".$pval['employee_id']."','".$pval['dispo_code']."','".$pval['executive_summary']."','".$pval['executive_other']."','".$pval['executive_location_lat']."','".$pval['executive_location_long']."','".$pval['executive_address']."','".$today."','".$pval['executive_transtype']."')";
    $result2 = mysqli_query($conn,$sql2);
        $last_id = mysqli_insert_id($conn);
    	}
    	else{
    	    $sql2 = "update executive_details set dispo_code='".$pval['dispo_code']."',executive_summary='".$pval['executive_summary']."',executive_other='".$pval['executive_other']."',executive_location_lat='".$pval['executive_location_lat']."',executive_location_long='".$pval['executive_location_long']."',executive_address='".$pval['executive_address']."',executive_time='".$today."',executive_transtype='".$pval['executive_transtype']."' where team_leader_id='".$pval['status_id']."'";
            $result2 = mysqli_query($conn,$sql2);
    	}
        ...
        $result3 = mysqli_query($conn,$sql3);
    	    ...
    	    $sql4 = "INSERT INTO cash_details(exect_det_id,summary,amount,trans_id,bank,cheque_number,date)VALUES('".$last_id."','".$pval['summary']."','".$pval['amount']."','".$pval['trans_id']."','".$pval['bank']."','".$pval['cheque_number']."','".$pval['date']."')";
    	    $result4 = mysqli_query($conn,$sql4);

        /*if($pval['executive_transtype']=='Cheque'){
            $pval['summary'] = isset($_GET['summary']) ? $_GET['summary'] : $_POST['summary'];
            $pval['bank'] = isset($_GET['bank']) ? $_GET['bank'] : $_POST['bank'];
    	    $pval['amount'] = isset($_GET['amount']) ? $_GET['amount'] : $_POST['amount'];
            $pval['cheque_number'] = isset($_GET['cheque_number']) ? $_GET['cheque_number'] : $_POST['cheque_number'];
    	    ...
    	    $sql4 = "INSERT INTO chek_details(exect_det_id,summary,bank,amount,cheque_number,date)VALUES('".$last_id."','".$pval['summary']."','".$pval['bank']."','".$pval['amount']."','".$pval['cheque_number']."','".$pval['date']."')";
    	    $result4 = mysqli_query($conn,$sql4); 
        }
        
        if($pval['executive_transtype']=='Online Transaction'){
           $pval['summary'] = isset($_GET['summary']) ? $_GET['summary'] : $_POST['summary'];
    	    $pval['amount'] = isset($_GET['amount']) ? $_GET['amount'] : $_POST['amount'];
    	    $pval['trans_id'] = isset($_GET['trans_id']) ? $_GET['trans_id'] : $_POST['trans_id'];
    	    
    	    $sql4 = "INSERT INTO online_details(exect_det_id,summary,amount,trans_id)VALUES('".$last_id."','".$pval['summary']."','".$pval['amount']."','".$pval['trans_id']."')";
    	    $result4 = mysqli_query($conn,$sql4); 
        }*/
      ...
      for ($i = 1; $i <= $count; $i++) 
      {
        while($final_data= mysqli_fetch_array($result)){
            $array1 = [];
             ...
             $sql1 = "SELECT * FROM team_leader,executive_details WHERE team_leader.Id=executive_details.team_leader_id and executive_details.employee_id='".$Id."' and executive_details.dispo_code='PAID' order by executive_details.Id DESC LIMIT $offset, $rowsperpage";
